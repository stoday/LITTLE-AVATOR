"""Deterministic parsing and due detection for Markdown-backed reminders."""
...
from .notes import ActivityLog, Note, NoteStore
...
class ReminderScanner:
    """Scan fresh note data once, recording one outcome per due occurrence."""
            ...
            try:
                ...
                if outcome == "missed":
                    self.activity_log.append(
                        "skipped_missed", note_id=note.note_id, occurrence=occurrence, source="scheduler"
                    )
                elif not has_subscriber:
                    self.activity_log.append(
                        "skipped_no_client", note_id=note.note_id, occurrence=occurrence, source="scheduler"
                    )
                else:
                    due.append(DueReminder(note.note_id, occurrence, note.content))
            except Exception as exc:
                self.activity_log.append(
                    "notes_parse_error", note_id=note.note_id, source="scheduler", error=type(exc).__name__
                )
        return due
    ...
    def _occurrence(note: Note, now: datetime) -> tuple[str | None, str | None]:
        ...
        if kind == "daily":
            time_text = str(reminder["time"])
            due = datetime.strptime(time_text, "%H:%M").time()
            ...
            if reminder.get("last_handled_occurrence") == occurrence or now < scheduled:
                return None, None
            return occurrence, "due" if now.strftime("%H:%M") == time_text else "missed"
        if kind == "once":
