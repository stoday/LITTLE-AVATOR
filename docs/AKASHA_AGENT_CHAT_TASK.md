...
- The model alias comes from the existing `MODEL` environment variable. The current expected value is `gemini:gemini-2.5-flash`; provider credentials, including `GEMINI_API_KEY`, remain environment-only and are never logged or returned by an endpoint.
